...
insert transformed an cleaned data into silver tables
...
BEGIN TRY
	  ...
	  SET @batch_start_time = GETDATE(); 
		...
		*,
		...
		SELECT
			...
			----converting data from one to another
			...
			AS prd_end_dt
		FROM bronze.crm_prd_info ---calculate end date as one day before the text start date
		...
		CASE WHEN TRIM(cntry) = 'DE' THEN 'Germany'
			 ...
			 ELSE TRIM(cntry) --data normalisation, remove unwanted table
		END AS cntry
	...
	SET @start_time = GETDATE();
		...
		CASE WHEN UPPER(TRIM(gen)) IN ('F', 'FEMALE') THEN 'Female'
			 ...
			 ELSE 'n/a'
			END AS gen ---normalize gender values and handle unknwon values
